...
function conversion(b1::Ultraspherical,b2::Ultraspherical)
    ...
    # L0
    if b1.λ ≈ b2.λ
         return ConcreteLazyOperator(b1,b2,BasicBandedOperator(0,0,(i,j) -> Float64(i == j)))
    end

    λ0 = b1.λ
    L0 = BasicBandedOperator(0,2,_conv_ultra(λ0))
    λ0 += 1
    while !(λ0 - b2.λ ≈ 0)
        L0 = BasicBandedOperator(0,2,_conv_ultra(λ0))*L0
        λ0 += 1
    end
